...
if(isset($_SESSION['language']) == TRUE) {
	include_once('languages/'.$_SESSION['language'].'/lng_install.php');
	include_once('languages/'.$_SESSION['language'].'/lng_main.php');
}
else {
	include_once('languages/de/lng_install.php');
	include_once('languages/de/lng_main.php');
}
...
switch(@$_GET['step']) {
	default:
		...
		if(isset($_GET['doit'])) {
			if(is_dir('languages/'.$p_language) == TRUE) {
		...
		while($akt_item = readdir($dp)) {
			if($akt_item != '.' && $akt_item != '..' && is_dir('languages/'.$akt_item) == TRUE && file_exists('languages/'.$akt_item.'/language.cfg') == TRUE) {
				$cur_lng_config = parse_ini_file('languages/'.$akt_item.'/language.cfg');
				echo '<option value="'.$akt_item.'">'.$cur_lng_config['language_name_native'].' ('.$cur_lng_config['language_name'].')</option>';
			}
		}
		closedir($dp);
				...
				if(isset($_GET['doit'])) {
					...
					elseif(isset($_POST['p_action'])) {
						...
						elseif($_POST['p_action'] == '3') {
							...
							$DB->query("UPDATE ".TBLPFX."config SET config_value='".$SCRIPTVERSION."' WHERE config_name='dataversion'");
