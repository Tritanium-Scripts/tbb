...
function unify_nick($nick) {
	...
	$DB->query("SELECT user_id FROM ".TBLPFX."users WHERE user_nick='$nick' LIMIT 1");
...
function get_user_data($user_id) {
	...
	if(!preg_match('/^[0-9]{1,}$/si',$user_id))
		$DB->query("SELECT * FROM ".TBLPFX."users WHERE user_nick='$user_id' LIMIT 1");
	else $DB->query("SELECT * FROM ".TBLPFX."users WHERE user_id='$user_id' LIMIT 1");
...
function get_forum_data($forum_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."forums WHERE forum_id='$forum_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_user_id($user_id) {
	...
	if(!preg_match('/^[0-9]{1,}$/si',$user_id))
		$DB->query("SELECT user_id FROM ".TBLPFX."users WHERE user_nick='$user_id' LIMIT 1");
	else $DB->query("SELECT user_id FROM ".TBLPFX."users WHERE user_id='$user_id' LIMIT 1");
...
function get_topic_data($topic_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."topics WHERE topic_id='$topic_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_smiley_data($smiley_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."smilies WHERE smiley_id='$smiley_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_post_data($post_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."posts WHERE post_id='$post_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_group_data($group_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."groups WHERE group_id='$group_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_poll_data($poll_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."polls WHERE poll_id='$poll_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_rank_data($rank_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."ranks WHERE rank_id='$rank_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}
...
function get_avatar_data($avatar_id) {
	...
	$DB->query("SELECT * FROM ".TBLPFX."avatars WHERE avatar_id='$avatar_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;
}

function get_profile_note_data($note_id) {
	global $DB;

	$DB->query("SELECT * FROM ".TBLPFX."profile_notes WHERE note_id='$note_id' LIMIT 1");
	return ($DB->affected_rows == 1) ? $DB->fetch_array() : FALSE;	
}
