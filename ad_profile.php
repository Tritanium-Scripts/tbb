...
switch(@$_GET['mode']) {
	...
	case 'addfield':
		...
		$p_field_link = isset($_POST['p_field_link']) ? $_POST['p_field_link'] : '%1$s';
		$p_field_data = array();

		if($p_field_link == '') $p_field_link = '%1$s';
		...
		if(isset($_GET['doit'])) {
			$DB->query("INSERT INTO ".TBLPFX."profile_fields (field_name,field_type,field_is_required,field_show_registration,field_show_memberlist,field_data,field_regex_verification,field_link) VALUES ('$p_field_name','$p_field_type','$p_field_is_required','$p_field_show_registration','$p_field_show_memberlist','".serialize($p_field_data)."','$p_field_regex_verification','$p_field_link')");
	...
	case 'editfield':
		...
		$p_field_link = isset($_POST['p_field_link']) ? $_POST['p_field_link'] : addslashes($field_data['field_link']);
