...
switch(@$_GET['mode']) {
		...
		if($cats_counter != 0) { // Falls auch eine Kategorie existiert
			for($i = 0; $i < $cats_counter; $i++) {
				...
				for($j = 0; $j < $forums_counter; $j++) {
					if($forums_data[$j]['cat_id'] == $akt_cat['cat_id']) $akt_forums_counter++; // Falls Forum zur aktuellen Kateogrie gehoert, Zaehler um 1 erhoehen
				}

				for($j = 0; $j < $forums_counter; $j++) {
					if($forums_data[$j]['cat_id'] == $akt_cat['cat_id']) { // Falls Forum zur aktuellen Kategorie gehoert
						$akt_forum = &$forums_data[$j];

						if($x == 1) $akt_forum_up = $lng['moveup']; // Falls Forum an 1. Stelle, kein "nach oben"-Link...
						else $akt_forum_up = "<a href=\"administration.php?faction=ad_forums&amp;mode=moveforumup&amp;forum_id_1=".$forums_data[$j]['forum_id']."&amp;forum_id_2=".$forums_data[$j-1]['forum_id']."&amp;$MYSID\">".$lng['moveup']."</a>"; // ...ansonsten Link erstellen

						if($x == $akt_forums_counter) $akt_forum_down = $lng['movedown']; // Falls Forum an letzter Stelle, kein "nach unten" Link...
						else $akt_forum_down = "<a href=\"administration.php?faction=ad_forums&amp;mode=moveforumdown&amp;forum_id_1=".$forums_data[$j]['forum_id']."&amp;forum_id_2=".$forums_data[$j+1]['forum_id']."&amp;$MYSID\">".$lng['movedown']."</a>"; // ...ansonsten Link erstellen
	...
	case 'editcat':
		...
		if($x == FALSE) $adforums_tpl->unset_block('optionrow');
