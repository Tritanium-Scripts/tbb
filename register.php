...
if($mode == "createuser") {
	$newuser_pw1 = mysslashes($HTTP_POST_VARS['newuser_pw1']);
	$newuser_pw2 = mysslashes($HTTP_POST_VARS['newuser_pw2']);
	$nu_mailname = mysslashes($newuser_name);
	$signatur = nlbr(trim(mutate($HTTP_POST_VARS['signatur']))); $newuser_email = trim(mutate($HTTP_POST_VARS['newuser_email'])); // Signatur und HTML-Mailadresse kompatibel machen
	$newuser_name = mutate(str_replace(' ','_',$HTTP_POST_VARS['newuser_name']));
	...
	elseif($regeln != "yes") $fehler = $lng['register']['error']['Boardrules_not_accepted']; // Überprüfen, ob die Regeln akzeptiert wurden
		...
		$newuser_hp = $HTTP_POST_VARS['newuser_hp'];
		$newuser_icq = $HTTP_POST_VARS['newuser_icq'];
		$newuser_realname = $HTTP_POST_VARS['newuser_realname'];
