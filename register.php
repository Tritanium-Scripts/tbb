...
if(isset($_GET['doit'])) {
	...
	elseif($p_user_pw != $p_user_pw_confirmation && ($CONFIG['verify_email_address'] != 1 || $CONFIG['enable_email_functions'] != 1)) $error = $LNG['error_pws_no_match']; // Stimmen die Passworter ueberein?
		...
		$p_is_admin = (get_user_counter() == 0) ? 1 : 0;
		...
		$p_status = 1; // bedeutet, der User ist freigeschaltet
		$p_user_hash = '';
		if($p_is_admin != 1) {
			...
			elseif($CONFIG['verify_email_address'] == 2 && $CONFIG['enable_email_functions'] == 1) {
				$p_status = 0; // bedeutet, der User ist noch _nicht_ freigeschaltet
				$p_user_hash = get_rand_string(32); // ist später der Verifizierungscode
			}
		}

		$p_user_pwc = mycrypt($p_user_pw); // Passwort fuer Datenbank verschluesseln
		...
		if($p_is_admin != 1 && $CONFIG['enable_email_functions'] == 1) {
			...
			if($CONFIG['verify_email_address'] == 2) {
				$activation_link = $CONFIG['board_address'].'/index.php?faction=activateaccount&account_id='.$p_user_nick.'&activation_code='.$p_user_hash.'&doit=1';
		...
		$NAVBAR->addElements('left',array($LNG['Registration_successful'],''));
...
//
$NAVBAR->addElements('left',array($LNG['Register'],"index.php?faction=register&amp;$MYSID"));
