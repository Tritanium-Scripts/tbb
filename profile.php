...
elseif($profile_data['status'] == 5) { // Nur weitermachen, wenn User nicht gelöscht ist
	...
	case "edit":
		...
		elseif($user_id != $profile_id && $user_data['status'] != 1) echo $lng['No_access']; // Falls man nicht Admin oder "Eigentümer" des Profiles ist
			...
			isset($_POST['new_mail']) ? $n_mail = nlbr(trim(mutate($_POST['new_mail']))) : $n_mail = $profile_data['email'];
			isset($_POST['new_hp']) ? $n_hp = nlbr($_POST['new_hp']) : $n_hp = $profile_data['hp'];
			isset($_POST['new_pic']) ? $n_pic = nlbr($_POST['new_pic']) : $n_pic = $profile_data['pic'];
			isset($_POST['new_realname']) ? $n_realname = nlbr($_POST['new_realname']) : $n_realname = $profile_data['name'];
			isset($_POST['new_icq']) ? $n_icq = nlbr($_POST['new_icq']) : $n_icq = $profile_data['icq'];
			isset($_POST['new_signatur']) ? $n_signatur = nlbr(mutate($_POST['new_signatur'])) : $n_signatur = $profile_data['signatur'];
			isset($_POST['new_pw1']) ? $n_pw1 = nlbr($_POST['new_pw1']) : $n_pw1 = '';
			isset($_POST['new_pw2']) ? $n_pw2 = nlbr($_POST['new_pw2']) : $n_pw2 = '';

			$n_mail1 = $profile_data['showemail'];
			$n_mail2 = $profile_data['forummails'];
			...
			if($change == 1) {
				if(isset($delete)) { // Wurde "Account löschen" angeklickt?
					...
					isset($_POST['new_mail1']) ? $n_mail1 = 1 : $n_mail1 = 0;
					isset($_POST['new_mail2']) ? $n_mail2 = 1 : $n_mail2 = 0;

					$n_pw1 = mysslashes($n_pw1); $n_pw2 = mysslashes($n_pw2);
					if($n_mail == "") $mailfehler = $lng['profile']['error']['No_email']; // Es muss eine Mailadresse eingegeben werden
					elseif(!verify_email($n_mail)) $mailfehler = 'Bitte geben sie eine gültige Emailadresse an!';
					elseif($n_icq != '' && verify_icq_uin($n_icq) == FALSE) $mailfehler = 'Bitte geben Sie eine gültige ICQ-Nummer an!';
					elseif($n_pw1 != "" && $n_pw1 != $n_pw2) $pwfehler = $lng['profile']['error']['Password_doesnt_match']; // Die Passwörter werden verglichen
					else {
						$showformular = 0;

						if($n_pw1 == '') $n_pw = $profile_data['pw'];
						else $n_pw = mycrypt($n_pw1); // Altes Passwort verwenden oder neues verschlüsseln

						$user_file = myfile("members/$profile_id.xbb"); $user_file_size = sizeof($user_file);
						$user_file[2] = $n_pw."\n"; // Neues Paswort
						$user_file[3] = $n_mail."\n"; // Neue Mailadresse
						$user_file[7] = $n_signatur."\n"; // Neue Signatur
						$user_file[9] = $new_hp."\n"; // Neue HP-Adresse
						$user_file[10] = $n_pic."\n"; // Neues Bild
						$user_file[12] = $n_realname."\n"; // Neuer Name
						$user_file[13] = $n_icq."\n"; // Neue ICQ-Nummer
						$user_file[14] = $n_mail1.','.$n_mail2."\n"; // Neue Mailoptionen
			...
			if ($showformular == 1) {
				...
				<tr>
				 ...
				 <td class="td1" width="80%"><input type="text" name="new_mail" value="<?=$n_mail?>"><span class="error"><?=$mailfehler?></span></td>
				</tr>
				<tr>
				...
				<tr>
				 ...
				 <td class="td1" width="80%"><input type="text" name="new_hp" value="<?=$n_hp?>"></td>
				</tr>
				<tr>
				 <td class="td1" width="20%"><span class="norm"><b><?=$lng['profile']['Avatar']?>:</b></span></td>
				 <td class="td1" width="80%"><input type="text" name="new_pic" value="<?=$n_pic?>"></td>
				</tr>
				<tr>
				 <td class="td1" width="20%"><span class="norm"><b><?=$lng['profile']['Real_name']?>:</b></span></td>
				 <td class="td1" width="80%"><input type="text" name="new_realname" value="<?=$n_realname?>"></td>
				</tr>
				<tr>
				 <td class="td1" width="20%"><span class="norm"><b><?=$lng['profile']['ICQ']?>:</b></span></td>
				 <td class="td1" width="80%"><input type="text" name="new_icq" value="<?=$n_icq?>"></td>
				</tr>
				<tr>
				 ...
				 <td class="td1" width="80%"><textarea cols="50" rows="6" name="new_signatur"><?=brnl($n_signatur)?></textarea></td>
				</tr>
				<tr><td class="kat" colspan="2"><span class="kat"><?=$lng['Options']?></span></td></tr>
				<tr><td class="td1" colspan="2"><input type="checkbox" value="1" name="new_mail2"<? if($n_mail1 == 1) echo " checked" ?>> <span class="norm"><?=$lng['profile']['Show_emailaddress']?></span></td></tr>
				<tr><td class="td1" colspan="2"><input type="checkbox" value="1" name="new_mail1"<? if($n_mail2 == 1) echo " checked" ?>> <span class="norm"><?=$lng['profile']['Receive_mails_from_forum']?></span></td></tr>
