			...
			if($change == 1) {
				if(isset($delete)) { // Wurde "Account löschen" angeklickt?
					...
					elseif($new_pw1 != "" && $new_pw1 != $new_pw2) $pwfehler = $lng['profile']['error']['Password_doesnt_match']; // Die Passwörter werden verglichen
						...
						$showformular = 0;
						$new_signatur = nlbr(trim(mutate($new_signatur))); // Signatur wird kompatibel gemacht
						if($new_pw1 == "") $new_pw = $profile_data[pw]; else $new_pw = mycrypt($new_pw1, "Xb"); // Altes Passwort verwenden oder neues verschlüsseln
						$user_file = myfile("members/$profile_id.xbb"); $user_file_size = sizeof($user_file);
						$user_file[2] = "$new_pw\r\n"; // Neues Paswort
						$user_file[3] = "$new_mail\r\n"; // Neue Mailadresse
						$user_file[7] = "$new_signatur\r\n"; // Neue Signatur
						$user_file[9] = "$new_hp\r\n"; // Neue HP-Adresse
						$user_file[10] = "$new_pic\r\n"; // Neues Bild
						$user_file[12] = "$new_realname\r\n"; // Neuer Name
						$user_file[13] = "$new_icq\r\n"; // Neue ICQ-Nummer
						$user_file[14] = "$new_mail1,$new_mail2\r\n"; // Neue Mailoptionen
