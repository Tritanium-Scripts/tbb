...
if($topics_counter == 0) {
	...
	$db->raw2array();
	$ppics_data = $db->array_data;

	$viewforum_tpl->unset_block('no_topics');

	$db->query("SELECT t1.*,UNIX_TIMESTAMP(t2.post_time) AS topic_last_post_time, t2.poster_id AS topic_last_post_poster_id,t3.user_nick AS topic_poster_nick, t4.user_nick AS topic_last_post_poster_nick FROM ".TBLPFX."topics AS t1, ".TBLPFX."posts AS t2 LEFT JOIN ".TBLPFX."users AS t3 ON t1.poster_id=t3.user_id LEFT JOIN ".TBLPFX."users AS t4 ON t2.poster_id=t4.user_id WHERE t1.topic_last_post_id=t2.post_id ORDER BY t2.post_time DESC LIMIT $start,".$CONFIG['topics_per_page']);
