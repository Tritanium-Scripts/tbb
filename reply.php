...
if($right == 1) {
	...
	if($mode == "save" && !isset($preview)) {
		if($user_logged_in != 1 && trim($nli_name) == '' && $config['nli_must_enter_name'] == 1) $error = $lng['replyf']['error']['Please_enter_a_name'];
		elseif(trim($post) == '') $error = $lng['replyf']['error']['Please_enter_a_post_text'];
		else {
			$post = preg_replace("/[?&]sid=[a-z0-9]{32}/","",$post);
			$showformular = 0;
			...
			// Hier beginnt der Sendmail Abschnitt
				if($topic_data['smstatus'] == 1 && $config['activate_mail'] == 1 && $config['notify_new_replies'] == 1 && $user_id != $topic_data['creator_id']) {
		...
		if($forum_data['upbcode'] == 1) {
	 	...
	 	?>
			<tr>
			 ...
			 <td class="td1" width="80%"><textarea name="post" rows="10" cols="60"><?=$quote?><?=mutate($post)?></textarea></td>
			</tr>
