	...
	public function parse($string, $enableHTML=false, $enableSmilies=true, $enableBBCode=true, &$topic=array())
	{
		...
		if($enableBBCode)
		{
			...
			while(preg_match("/\[quote\](.*?)\[\/quote\]/si", $string))
				$string = preg_replace_callback("/\[quote\](.*?)\[\/quote\][\r\n]*/si", create_function('$elements', 'return Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_QUOTE, \'quoteText\' => $elements[1], \'quoteTitle\' => Main::getModule(\'Language\')->getString(\'quote\')));'), $string);
			while(preg_match("/\[quote=(.*?)\](.*?)\[\/quote\]/si", $string))
				$string = preg_replace_callback("/\[quote=(.*?)\](.*?)\[\/quote\][\r\n]*/si", create_function('$elements', 'return Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_QUOTE, \'quoteText\' => $elements[2], \'quoteTitle\' => sprintf(Main::getModule(\'Language\')->getString(\'quote_by_x\'), $elements[1])));'), $string);
		}
		return $string;
	}
