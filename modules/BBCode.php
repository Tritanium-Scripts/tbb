...
class BBCode
{
	...
	public function parse($string, $enableHTML=false, $enableSmilies=true, $enableBBCode=true, &$topic=array())
	{
		...
		if($enableBBCode)
		{
			...
			$string = preg_replace_callback("/\[php\](.*?)\[\/php\]/si", create_function('$elements', 'return Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_CODE, \'codeLines\' => Functions::str_replace(array(\'<code>\', \'</code>\'), \'\', highlight_string(Functions::br2nl(htmlspecialchars_decode($elements[1])), true))));'), $string);
			...
			#$string = preg_replace_callback("/\[img=(\d+),(\d+)\](.*?)\[\/img\]/si", create_function('$elements', 'return Functions::substr($elements[3], 0, 11) == \'javascript:\' ? $elements[0] : Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_IMAGE, \'imageAddress\' => $elements[3], \'imageText\' => \'\', \'imageHeight\' => $elements[2], \'imageWidth\' => $elements[1]));'), $string);
			...
			$string = preg_replace_callback("/\[iframe\](.*?)\[\/iframe\]/si", create_function('$elements', 'return Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_IFRAME, \'iFrameLink\' => $elements[1], \'iFrameWidth\' => 560, \'iFrameHeight\' => 315));'), $string);
			$string = preg_replace_callback("/\[iframe=(\d+),(\d+)\](.*?)\[\/iframe\]/si", create_function('$elements', 'return Functions::substr($elements[3], 0, 11) == \'javascript:\' ? $elements[3] : Main::getModule(\'Template\')->fetch(\'BBCode\', array(\'type\' => BBCODE_IFRAME, \'iFrameLink\' => $elements[3], \'iFrameWidth\' => $elements[1], \'iFrameHeight\' => $elements[2]));'), $string);
