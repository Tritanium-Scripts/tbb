		...
		for($i = 0; $i < $postsCounter; $i++) {
			...
			if($curPost['posterID'] == 0) {
				...
				elseif($curPost['postPosterIsAdmin'] == 1) { // Falls der User Administrator ist...
					$curPosterRankText = $this->modules['Language']->getString('administrator'); // ...seinen Rang darauf setzen...
				...
				elseif($curPost['postPosterIsSupermod'] == 1) { // Falls der User Supermoderator ist...
					$curPosterRankText = $this->modules['Language']->getString('supermoderator'); // ...seinen Rang darauf setzen...
				...
				elseif(isset($forumModIDs[$curPost['posterID']]) == TRUE) { // Falls der User Moderator ist...
					$curPosterRankText = $this->modules['Language']->getString('moderator'); // ...seinen Rang darauf setzen...
			...
			if($curPost['postShowSignature'] == 1 && $this->modules['Config']->getValue('enable_sig') == 1 && $curPost['postPosterSignature'] != '') {
				if(!isset($parsedSignatures[$curPost['posterID']])) // Falls die Signatur nicht schonmal formatiert wurde
					$parsedSignatures[$curPost['posterID']] = $this->modules['BBCode']->format($curPost['postPosterSignature'], ($this->modules['Config']->getValue('allow_sig_html') == 1), ($this->modules['Config']->getValue('allow_sig_smilies') == 1), ($this->modules['Config']->getValue('allow_sig_bbcode') == 1));
