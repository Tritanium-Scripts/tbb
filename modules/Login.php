				...
				else
				{
					...
					foreach(glob(DATAPATH . 'members/[!0]*.xbb') as $curMember)
					{
						...
						if($this->loginName == Functions::strtolower($curMember[0]))
						{
							...
							Functions::file_put_contents('members/' . $curMember[1]. '.xbb', implode("\n", $curMember));
							if(!Functions::sendMessage($curMember[3], 'new_password_requested', $_SERVER['REMOTE_ADDR'], $this->loginName, $newPass, Main::getModule('Config')->getCfgVal('address_to_forum') . '/' . INDEXFILE . '?faction=login'))
