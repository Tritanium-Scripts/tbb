 ...
 * @copyright Copyright (c) 2021 Tritanium Scripts
...
class Calendar implements Module
{
	...
	{
		Main::getModule('NavBar')->addElement(Main::getModule('Language')->getString('calendar'), INDEXFILE . '?faction=calendar' . SID_AMPER);
		if(Main::getModule('Config')->getCfgVal('activate_calendar') != 1)
		...
		foreach(glob(DATAPATH . 'members/[!0t]*.xbb') as $curMember)
		{
			...
			if($curRegYear <= $this->year && $curRegMonth == $this->month)
			{
				...
				switch($reggedYears)
				{
					...
					$curEvent['description'] = Main::getModule('Language')->getString('registration');
					break;

					case 1:
					$curEvent['icon'] = 'anniversary';
					$curEvent['description'] = Main::getModule('Language')->getString('anniversary_one_year');
					break;

					default:
					$curEvent['icon'] = 'anniversary';
					$curEvent['description'] = sprintf(Main::getModule('Language')->getString('anniversary_x_years'), $reggedYears);
					break;
				}
		...
		Main::getModule('Template')->printPage('Calendar', array('year' => $this->year,
			'month' => $this->month,
			'date' => $date,
			'calendar' => $calendar));
	}
