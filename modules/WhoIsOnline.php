    ...
    public function publicCall(): void
    {
        ...
        foreach(Functions::file('vars/wio.var') as $curWIOEntry)
        {
            ...
            if(!($curWIOEntryIsGhost = $curWIOEntry[4] == '1') || Auth::getInstance()->isAdmin())
            {
                ...
                {
                    case 'ForumIndex':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_the_forum_index'), INDEXFILE . SID_QMARK), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;
                    ...
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_todays_posts'), INDEXFILE . '?faction=todaysPosts' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'RSSFeed':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_the_rss_feed'), INDEXFILE . '?faction=rssFeed' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'WhoIsOnline':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_the_wio_list'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'MemberList':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_the_member_list'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Message':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_a_message'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Login':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('logs_in'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'RequestPassword':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('requests_a_new_password'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'ViewProfile':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_the_profile_from_x'), Functions::getProfileLink($curWIOEntry[2][1])), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'vCard':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('downloads_the_vcard_from_x'), Functions::getProfileLink($curWIOEntry[2][1])), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'SendMail':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('writes_a_mail_to_x'), Functions::getProfileLink($curWIOEntry[2][1])), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'ViewAchievements':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_achievements_from_x'), Functions::getProfileLink($curWIOEntry[2][1])), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'EditProfile':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_own_profile'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'EditProfileConfirmDelete':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('deletes_own_account'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Register':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('registers'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'BoardRules':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('reads_board_rules'), INDEXFILE . '?faction=regeln' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'FAQ':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_the_faq'), INDEXFILE . '?faction=faq' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'GDPR':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('reads_privacy_policy'), INDEXFILE . '?faction=gdpr' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Credits':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_the_credits'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'PrivateMessageIndex':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_pms'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'PrivateMessageViewPM':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('reads_a_pm'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'PrivateMessageNewPM':
                    case 'PrivateMessageNewPMConfirmSend':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('writes_new_pm'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'PrivateMessageConfirmDelete':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('deletes_a_pm'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;
                    ...
                    case 'Search':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('searches_the_board'), INDEXFILE . '?faction=search' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'SearchResults':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_search_results'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Newsletter':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('is_in_newsletter_archive'), INDEXFILE . '?faction=newsletter' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'NewsletterReadLetter':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('reads_a_newsletter'), INDEXFILE . '?faction=newsletter&amp;mode=read&amp;newsletter=' . $curWIOEntry[2][1] . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Calendar':
                    $wioLocations[] = array($curUser, sprintf(Language::getInstance()->getString('views_the_calendar'), INDEXFILE . '?faction=calendar' . SID_AMPER), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'Upload':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('uploads_a_file'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminIndex':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('is_in_administration'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForum':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_forums_categories'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumIndex':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_forums'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumEditForum':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_a_forum'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumDeleteForum':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('deletes_a_forum'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumNewForum':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_a_new_forum'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumSpecialRights':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_special_rights'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumNewUserRight':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_user_special_right'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumNewGroupRight':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_group_special_right'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumIndexCat':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_categories'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumEditCat':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_a_category'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminForumNewCat':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_category'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminRankIndex':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_user_ranks'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminRankEditRank':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_an_user_rank'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminRankNewRank':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_user_rank'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminConfig':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_board_settings'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminConfigResetConfirm':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('resets_board_settings'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminConfigCountersConfirm':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('recalculates_counters'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminLogfile':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_logfiles'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminLogfileViewLog':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('views_a_logfile'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminTemplate':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_templates'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminNews':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('writes_board_news'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminMailList':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('retrieves_mail_list'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminUser':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_user'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminUserNewUser':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_user'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminUserEditUser':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_an_user'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminGroup':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_groups'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminGroupNewGroup':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_group'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminGroupEditGroup':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_a_group'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminGroupDeleteGroup':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('deletes_a_group'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminCensor':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_censorships'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminCensorNewWord':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_censorship'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminCensorEditWord':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_a_censorship'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminIP':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_ip_blocks'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminIPNewBlock':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_ip_block'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminSmiley':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('manages_smilies'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminSmileyNewSmiley':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('creates_new_smiley'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminSmileyEditSmiley':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('edits_a_smiley'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;

                    case 'AdminNewsletter':
                    case 'AdminNewsletterConfirm':
                    $wioLocations[] = array($curUser, Language::getInstance()->getString('writes_a_newsletter'), $curWIOEntryIsGhost, $curTime, $curWIOEntry[5]);
                    break;
