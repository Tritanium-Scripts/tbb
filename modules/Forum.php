...
//ViewTopic
			...
			for($i=$end-Main::getModule('Config')->getCfgVal('posts_per_page'); $i<($end > $size ? $size : $end); $i++)
			{
				...
				$posts[] = $curPoster + array('postID' => $curPost[0],
					...
					'canModify' => Main::getModule('Auth')->isAdmin() || $isMod || (Main::getModule('Auth')->isLoggedIn() && Main::getModule('Auth')->getUserID() == $curPost[1] && Functions::checkUserAccess($forum, 4)/* && (Main::getModule('Config')->getCfgVal('edit_time') == 0 || Functions::getTimestamp(gmdate('YmdHis')) < Functions::getTimestamp($curPost[2])+Main::getModule('Config')->getCfgVal('edit_time'))*/),
