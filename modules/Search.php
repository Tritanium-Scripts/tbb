 ...
 * @package TBB1
 */
class Search extends PublicModule
{
    ...
    {
        NavBar::getInstance()->addElement(Language::getInstance()->getString('search'), INDEXFILE . '?faction=search' . SID_AMPER);
        ...
        if(isset($_SESSION[$this->searchID]))
        {
            ...
            {
                NavBar::getInstance()->addElement(Language::getInstance()->getString('search_results'), INDEXFILE . '?faction=search&amp;searchID=' . $this->searchID . SID_AMPER);
                $results = $idTable = [];
                $topicCounter = 0;
                    ...
                    foreach($curTopics as $curTopicID => $curPosts)
                    {
                        ...
                        foreach($curPosts as $curPostID)
                        {
                            if($curPostID == 0)
                            {
                                ...
                                foreach(array_map(['Functions', 'explodeByTab'], $curTopicFile) as $curKey => $curPost)
                                {
        ...
        else
        {
            ...
            if(Functions::getValueFromGlobals('search') == 'yes')
            {
                ...
                elseif($this->searchOption != 'user' && Functions::strlen($this->searchFor) < 3)
                    $this->errors[] = Language::getInstance()->getString('search_term_is_too_short');
