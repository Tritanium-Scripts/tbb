                    ...
                    foreach($achievements as $curAchievement)
                        if($curAchievement->attributes->getNamedItem('closed')->nodeValue == '1')
                            $achievementsClosed[] = ['icon' => $curAchievement->getElementsByTagName('iconClosed')->item(0)->nodeValue,
                                ...
                                'unlocked' => $curAchievement->getElementsByTagName('unlockTimestamp')->length == 1 ? Functions::gmstrftime(Language::getInstance()->getString('DATEFORMAT'), $curAchievement->getElementsByTagName('unlockTimestamp')->item(0)->nodeValue) : ''];
                        else
                    ...
                    foreach($achievements as $curAchievement)
                    {
                        ...
                        if($curAchievement['achieved'] == 1)
                            $achievementsClosed[] = ['icon' => $curAchievement['icon'],
                                ...
                                'unlocked' => $curAchievement['unlocktime'] > 0 ? Functions::gmstrftime(Language::getInstance()->getString('DATEFORMAT'), $curAchievement['unlocktime']) : ''];
                        else
