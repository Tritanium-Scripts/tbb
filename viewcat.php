...
// Template laden
//
$viewcat_tpl = new template;
$viewcat_tpl->load($template_path.'/'.$tpl_config['tpl_viewcat']);
...
for($i = 0; $i < $cats_counter; $i++) { // Jede Kategorie durchlaufen lassen
	...
	if(in_array($akt_cat['cat_id'],$open_cats) == TRUE) { // Falls diese Kategorie geöffnet sein soll...
		...
		$akt_plus_minus_pic = '<a href="index.php?faction=viewcat&amp;cat_id='.$cat_id.'&amp;close_cat='.$akt_cat['cat_id'].'&amp;'.$MYSID.'"><img src="'.$template_path.'/'.$tpl_config['img_minus'].'" border="0" alt="" /></a>';
		...
		$akt_plus_minus_pic = '<a href="index.php?faction=viewcat&amp;cat_id='.$cat_id.'&amp;open_cat='.$akt_cat['cat_id'].'&amp;'.$MYSID.'"><img src="'.$template_path.'/'.$tpl_config['img_plus'].'" border="0" alt="" /></a>'; // ...Bild fuer "Oeffnen" bestimmen
...
show_navbar("<a href=\"index.php?$MYSID\">".$CONFIG['board_name']."</a>".get_navbar_cats($cat_id,FALSE)."\r".myhtmlentities($cat_data['cat_name']));
