...
if($USER_LOGGED_IN == 1) {
	...
	//
	if($USER_DATA['user_is_admin'] != 1 && $USER_DATA['user_is_supermod'] != 1) {
	...
	$DB->query("SELECT voter_id FROM ".TBLPFX."polls_votes WHERE poll_id='$poll_id' AND voter_id='$USER_ID'",TRUE);
	if($DB->affected_rows == 0) {
		...
		if($DB->affected_rows == 1) {
			...
			$DB->query("INSERT INTO ".TBLPFX."polls_votes (poll_id,voter_id) VALUES ('$poll_id','$USER_ID')",TRUE);
		}
	}
}
