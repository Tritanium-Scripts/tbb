...
switch(@$_GET['mode']) {
	...
	case 'newpm':
		...
		if(isset($_GET['doit'])) {
			...
			if(sizeof($recipients) == 0) $error = $lng['error_no_recipient'];
		...
		if($CONFIG['allow_pms_bbcode'] == 1)	$pms_tpl->blocks['bbcodecheck']->parse_code(); //...BBCode...
		else $pms_tpl->unset_block('bbcodecheck');
	...
	case 'viewpm':
		...
		if($pm_data['pm_type'] == 0) {
			...
			if($CONFIG['allow_pms_bbcode'] == 1) $pms_tpl->blocks['replyform']->blocks['bbcodecheck']->parse_code(); //...BBCode...
			else $pms_tpl->blocks['replyform']->unset_block('bbcodecheck');
