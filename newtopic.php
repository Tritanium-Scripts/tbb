...
if($right == 1) {
	...
	if($save == "yes" && !$preview) {
		...
		elseif($post == "") $error = $lng['newtopic']['error']['no_post'];
		elseif($user_logged_in != 1 && trim($nli_name) == '' && $config['nli_must_enter_name'] == 1) $error = $lng['newtopic']['error']['Please_enter_a_name'];
		else {
			$showformular = 0;
	...
	if($showformular == 1) {
		...
		?>
			...
			<table width="<?=$twidth?>" class="tbl" border="0" cellspacing="<?=$tspacing?>" cellpadding="<?=$tpadding?>">
