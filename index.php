...
if($config['uc'] == 1 && $user_data['status'] != 1) {
	?>
		...
		<td><tr><center><img src="<?=$config['forum_logo']?>"><br></center></td></tr>
		</table>
	<?
...
elseif(check_ip_access($REMOTE_ADDR,-1) != 1) {
	...
	if($endtime == -1) $endtime = "Die Sperre gilt momentan für immer!";
	else {
		$endtime = ceil(($endtime - time()) / 60);
		$endtime = "Die Sperre gilt noch für $endtime Minuten";
	}

	include("pageheader.php");
	echo navbar("Zugriff verweigert");
	?>
		<table class="tbl" width="<?=$twidth?>" border="0" cellspacing="<?=$tspacing?>" cellpadding="<?=$tpadding?>">
		<tr><th class="thnorm"><span class="thnorm">Zugriff verweigert</span></th></tr>
		<tr><td class="td1"><span class="norm"><br><center>Ihre IP wurde für das gesamte Forum gesperrt!<br><?=$endtime?></center><br><br></span></td></tr>
		</table>
	<?
...
elseif($config['must_be_logged_in'] == 1 && $user_logged_in != 1 && $faction != "register" && $faction != "login" && $faction != "regeln" & $faction != "sendpw") {
	...
	echo navbar("Zugriff verweigert!");
	?>
		<table class="tbl" width="<?=$twidth?>" cellpadding="<?=$tpadding?>" cellspacing="<?=$tspacing?>" border="0">
		<tr><th class="thnorm"><span class="thnorm">Zugriff verweigert</span></th></tr>
		<tr><td class="td1"><span class="norm"><br><center>Dieses Forensystem können sie nur als eingeloggter, registrierter Benutzer betreten.<br><a href="index.php?faction=register<?=$MYSID2?>">Registrieren</a> sie sich oder <a href="index.php?faction=login<?=$MYSID2?>">loggen sie sich ein</a>.</center><br></span></td></tr>
		</table>
	<?
	include("pagetail.php");
	$allowed = 0;
	...
	switch($faction) {
		...
		default:
			if(check_ip_access($REMOTE_ADDR,$forum_id) != 1) {
				...
				include("pageheader.php");
				echo navbar("<a class=\"navbar\" href=\"index.php?mode=viewforum&forum_id=$forum_id$MYSID2\">".get_forum_name($forum_id)."</a>\tZugriff verweigert");
				?>
					<table class="tbl"width="<?=$twidth?>" cellpadding="<?=$tpadding?>" cellspacing="<?=$tspacing?>" border=0>
					<tr><th class="thnorm"><span class="thnorm">Zugriff verweigert</span></th>
					<tr><td class="td1"><span class="norm"><br><center>Ihre IP wurde für dieses Forum gesperrt!<br><?=$endtime?></center><br><br></span></td></tr>
					</table>
				<?
				include("pagetail.php");
			}
