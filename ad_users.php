...
//		$ad_users_tpl->blocks['nolockedusers']->parse_code();
	...
	case 'adduser':
		...
		if(isset($_GET['doit'])) {
			...
			if(trim($p_user_nick) == '' || verify_nick($p_user_nick) == FALSE) $error = $LNG['error_bad_nick'];
			elseif(unify_nick($p_user_nick) == FALSE) $error = $LNG['error_nick_already_in_use'];
			...
			elseif($p_user_pw1 != $p_user_pw2) $error = $LNG['error_pws_no_match'];
				...
				$p_user_pwc = mycrypt($p_user_pw1); // Passwort verschluesseln
				$p_user_pw = &$p_user_pw1; // Fuers Template...
				
				// Daten einfuegen
				$DB->query("INSERT INTO ".TBLPFX."users (user_nick,user_email,user_pw,user_regtime,user_tz) VALUES ('$p_user_nick','$p_user_email','$p_user_pwc','".time()."','".$CONFIG['standard_tz']."')");
				$new_user_id = $DB->insert_id;
				
				// Den neuesten User aktualisieren
				...
				include_once('pheader.php');
				show_message($LNG['User_added'],sprintf($LNG['message_new_user_added'],$p_user_nick),FALSE);
	...
	case 'edituser':
		...
		$p_user_auth_profile_notes = isset($_POST['p_user_auth_profile_notes']) ? intval($_POST['p_user_auth_profile_notes']) : $user_data['user_auth_profile_notes'];
		...
		if(isset($_GET['doit'])) {
			...
			if(trim($p_user_email) == '' || verify_email($p_user_email) == FALSE) $error = $LNG['error_bad_email'];
				...
				$DB->query("UPDATE ".TBLPFX."users SET
					user_is_admin='$p_user_is_admin',
					user_is_supermod='$p_user_is_supermod',
					user_email='$p_user_email',
					user_signature='$p_user_signature',
					user_avatar_address='$p_user_avatar_address'
