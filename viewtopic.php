...
$db->query("SELECT t1.post_id, t1.poster_id, t1.post_pic, t1.post_enable_bbcode, t1.post_enable_smilies, t1.post_enable_html, t1.post_show_sig, t1.post_guest_nick, t1.post_time, t1.post_edited_counter, t1.post_last_editor_id, t1.post_title, t1.post_text, t2.user_hp AS poster_hp, t2.user_email AS poster_email, t2.user_nick AS poster_nick, t2.user_signature AS poster_signature, t2.user_is_admin AS poster_is_admin, t2.user_posts AS poster_posts, t2.user_rank_id AS poster_rank_id, t2.user_avatar_address AS poster_avatar_address, t4.user_nick AS post_last_editor_nick FROM ".TBLPFX."posts AS t1 LEFT JOIN ".TBLPFX."users AS t2 ON t1.poster_id=t2.user_id LEFT JOIN ".TBLPFX."users AS t4 ON t1.post_last_editor_id=t4.user_id WHERE t1.topic_id='$topic_id' ORDER BY t1.post_time LIMIT $start,".$CONFIG['posts_per_page']);
while($akt_post = $db->fetch_array()) {
	...
	if($akt_post['poster_hp'] != '') $user_hp_button = '<a target="_blank" href="'.$akt_post['poster_hp'].'"><img src="'.$TEMPLATE_PATH.'/'.$TCONFIG['images']['user_hp'].'" alt="'.$akt_post['poster_hp'].'" border="0" /></a>';
	if($akt_post['poster_email'] != '') $user_email_button = '<a href="mailto:'.$akt_post['poster_email'].'"><img src="'.$TEMPLATE_PATH.'/'.$TCONFIG['images']['user_email'].'" alt="'.$akt_post['poster_email'].'" border="0" /></a>';
	...
	if($akt_post['poster_id'] == 0) { // Falls der Poster Gast bzw. ein geloeschter User ist...
		...
		elseif(in_array($akt_post['poster_id'],$forum_mod_ids) == TRUE) { // Falls der User Moderator ist...
			...
			while(list(,$akt_rank) = each($RANKS_DATA[0])) { // Die Rangliste durchlaufen
				if($akt_rank['rank_posts'] > $akt_post['poster_posts']) break;
...
add_navbar_items(array(myhtmlentities($forum_data['forum_name']),"index.php?faction=viewforum&amp;forum_id=$forum_id&amp$MYSID"),array(myhtmlentities($topic_data['topic_title']),''));

//
// Seite ausgeben
//
include_once('pheader.php');
show_navbar('','',$navbar_right);
