...
$DB->query("SELECT t1.post_id, t1.poster_id, t1.post_pic, t1.post_enable_bbcode, t1.post_enable_smilies, t1.post_enable_html, t1.post_show_sig, t1.post_guest_nick, t1.post_time, t1.post_edited_counter, t1.post_last_editor_id, t1.post_title, t1.post_text, t2.user_email AS poster_email, t2.user_nick AS poster_nick, t2.user_signature AS poster_signature, t2.user_is_admin AS poster_is_admin, t2.user_posts AS poster_posts, t2.user_rank_id AS poster_rank_id, t2.user_avatar_address AS poster_avatar_address, t4.user_nick AS post_last_editor_nick FROM ".TBLPFX."posts AS t1 LEFT JOIN ".TBLPFX."users AS t2 ON t1.poster_id=t2.user_id LEFT JOIN ".TBLPFX."users AS t4 ON t1.post_last_editor_id=t4.user_id WHERE t1.topic_id='$topic_id' ORDER BY t1.post_time LIMIT $start,".$CONFIG['posts_per_page']);
...
add_navbar_items(array(myhtmlentities($forum_data['forum_name']),"index.php?faction=viewforum&amp;forum_id=$forum_id&amp;$MYSID"),array(myhtmlentities($topic_data['topic_title']),''));
