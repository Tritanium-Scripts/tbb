...
switch(@$_GET['mode']) {
	...
	case 'edit':
		...
		$p_type = isset($_POST['p_type']) ? intval($_POST['p_type']) : $smiley_data['smiley_type'];
		$p_gfx = isset($_POST['p_gfx']) ? $_POST['p_gfx'] : addslashes($smiley_data['smiley_gfx']);
		$p_synonym = isset($_POST['p_synonym']) ? $_POST['p_synonym'] : addslashes($smiley_data['smiley_synonym']);
		$p_status = isset($_POST['p_status']) ? intval($_POST['p_status']) : $smiley_data['smiley_status'];
		...
		if(isset($_GET['doit'])) {
			...
			elseif($p_type == 0 && trim($p_synonym) == '') $error = $LNG['error_no_synonym'];
				...
				$DB->query("UPDATE ".TBLPFX."smilies SET smiley_type='$p_type', smiley_gfx='$p_gfx', smiley_status='".(($p_type == 1) ? 0 : $p_status)."', smiley_synonym='".(($p_type == 1) ? '' : $p_synonym)."' WHERE smiley_id='$smiley_id'");
				cache_set_smilies_data();
				cache_set_ppics_data();
				header("Location: administration.php?faction=ad_smilies&$MYSID"); exit;
			}
