...
/**
*
* Tritanium Bulletin Board 2 - db/filedb/functions.php
* Beinhaltet alle nötigen Funktionen um das Forum auf Dateibasis zu betreiben
...
function get_topics_counter($forum_id) {
...
function get_new_id($file) {
	...
	$new_id = fread($fp,filesize($file))+1; // Dateinhalt einlesen und um eins erhöhren
...
function get_post_data($topic_id,$post_id) {
	...
	if(sizeof($post_data) == 0) return FALSE;

	return $post_data;
}

function get_posts_counter($topic_id) {
...
function get_wio_data() {
	...
	while(list($akt_key) = each($data)) {
		if($data[$akt_key]['wio_user_id'] == 0) $data[$akt_key]['wio_user_nick'] = $lng['Guest'];
		elseif($akt_wio_user = get_user_data($data[$akt_key]['wio_user_id'])) $data[$akt_key]['wio_user_nick'] = $akt_wio_user['user_nick'];
		else $data[$akt_key]['wio_user_nick'] = $lng['Deleted_user'];
	}
	reset($data);
