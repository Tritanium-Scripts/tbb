...
if($CONFIG['enable_pms'] == 1 && $USER_LOGGED_IN == 1) {
	...
	$db->query("SELECT UNIX_TIMESTAMP(pm_send_time) AS last_pm_time FROM ".TBLPFX."pms WHERE pm_to_id='$USER_ID' AND pm_read_status='0' AND pm_send_time>'".unixtstamp2sqldatetime($_SESSION['s_last_pm_time'])."' ORDER BY pm_send_time DESC LIMIT 1");
