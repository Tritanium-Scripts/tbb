		...
		if(isset($_GET['doit'])) {
			...
			if($p_preview != 1) {
				...
				elseif(strlen($p_title) > 60) $error = $lng['error_title_too_long'];
					...
					$db->query("UPDATE ".TBLPFX."posts SET post_pic='$p_ppic_id', post_enable_bbcode='$p_bbcode', post_enable_smilies='$p_smilies', post_enable_html='$p_htmlcode', post_show_sig='$p_signature', post_edited_counter=post_edited_counter+1, post_last_editor_id='$USER_ID' WHERE post_id='$post_id'");
