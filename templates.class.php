...
class template {
	...
	function create_code($data,$prefix = '') {
		...
		$data = str_replace("'","\\'",$data);
		$data = preg_replace('/\{'.$prefix.'\$([a-zA-Z0-9_]+)([a-zA-Z0-9_\-\>\[\]\"\-]*)\}/',"'.\$this->get_var_value('\$GLOBALS[\"\\1\"]\\2').'",$data); // Die globalen Variablen
		$data = preg_replace('/\{'.$prefix.'([a-zA-Z0-9_]+)([a-zA-Z0-9_\[\]\"]*)\}/','\'.$this->values[\'\1\']\2.\'',$data); // Die lokalen Variablen
