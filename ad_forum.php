...
if($user_logged_in != 1 || $user_data['status'] != 1) {
	...
	case "newforum": // Neues Forum erstellen
		if($create != "yes") {
			...
			$towrite = "$neue_id\t$titel\t$description\t0\t0\t$kg\t$sm_mods\t$upbcode,$htmlcode\t\t\t$new_rights[0],$new_rights[1],$new_rights[2],$new_rights[3],$new_rights[4],$new_rights[5],$new_rights[6],$new_rights[7],$new_rights[8],$new_rights[9]\t$mods\t\t\t\t\r\n";
			...
			for($i = 0; $i < $foren_size; $i++) {
				...
				if($akt_forum[0] == $ad_forum_id) {
					...
					<tr><td class="kat" colspan=2><span class="kat"><?=$lng['Options']?></span></td></tr>
					 ...
					 <tr><td colspan="2" class="td1"><span class="norm"><input type="checkbox" name="sm_mods" value="1"<? if($akt_forum[6] == 1) echo " checked"; ?> onfocus="this.blur()"> <?=$lng['ad_forum']['Notify_mods_about_new_topics']?></span></td></tr>
			...
			if(isset($kill)) {
				...
				for($i = 0; $i < $foren_size; $i++) {
					...
					if($akt_forum[0] == $ad_forum_id) {
						...
						$akt_forum[8] = $op; $akt_forum[11] = $mods; $akt_forum[1] = $titel; $akt_forum[2] = $beschreibung; $akt_forum[5] = $kg; $akt_forum[6] = $sm_mods; $akt_forum[7] = "$upbcode,$htmlcode";
