		...
		if($topic_data[7] != '') { // Falls Umfrage existiert
			if(myfile_exists("polls/$topic_data[7]-1.xbb")) {
				...
				for($i = 1; $i < sizeof($poll_file); $i++) {
					...
					?>
						<?=$akt_poll[1]?></span></td><td><img src="images/poll.bmp" height="10" width="<?=round($votes)?>"></td><td><span class="small">(<?=$votes?> %, <?=$akt_poll[2]?> <?=$lng['readforum']['topic']['Votes']?>)</span></td></tr>
					<?
				}
