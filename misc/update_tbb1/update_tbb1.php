			...
			switch(@$_GET['substep']) {
				...
				case '2':
					...
					for($i = $start; $i < $end; $i++) {
						...
						$db->query("INSERT INTO ".TBLPFX."users (user_id,user_status,user_is_admin,user_nick,user_email,user_pw,user_posts,user_regtime,user_hp,user_icq,user_signature,user_tz) VALUES ('".$akt_user_data[1]."','1','".$akt_user_is_admin."','".mysql_escape_string($akt_user_data[0])."','".mysql_escape_string($akt_user_data[3])."','".$akt_user_data[2]."','".$akt_user_data[5]."','".$akt_user_data[6]."','".mysql_escape_string($akt_user_data[9])."','".$akt_user_data[13]."','".mysql_escape_string(tbb1_demutate(br2nl($akt_user_data[7])))."','gmt')");
						...
						if($akt_user_pms = myfile($_SESSION['s_tbb1_path'].'/members/'.$i.'.pm')) {
							while(list(,$akt_pm) = each($akt_user_pms)) {
								...
								$db->query("INSERT INTO ".TBLPFX."pms (pm_from_id,pm_to_id,pm_subject,pm_text,pm_read_status,pm_send_time,pm_enable_bbcode,pm_enable_smilies,pm_guest_nick) VALUES ('".$akt_pm_from_id."','".$akt_user_data[1]."','".mysql_escape_string(tbb1_demutate($akt_pm[1]))."','".mysql_escape_string(tbb1_demutate(br2nl($akt_pm[2])))."','".$akt_pm[7]."','$akt_pm_time','".$akt_pm[5]."','".$akt_pm[6]."','$akt_pm_guest_nick')");
							}
						}
...
<br /><br /><div align="center"><table class="tbl" cellspacing="1" cellpadding="3"><tr><td class="tdwhite"><span class="fontsmall">Tritanium Bulletin Board 2 Updatescript<br />&copy; 2003-2004 <a href="http://www.tritanium-scripts.com" target="_blank">Tritanium Scripts</a></span></td></tr></table></div>
</body>
</html><?php
}
