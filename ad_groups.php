...
switch(@$_GET['mode']) {
	...
	//*
	default:
		$ad_groups_tpl = new template; // Neues Template-Objekt erstellen
		$ad_groups_tpl->load($template_path.'/'.$tpl_config['tpl_ad_groups_overview']); // Template laden
		...
		else $ad_groups_tpl->unset_block('grouprow'); // Falls keine Gruppe, Block komplett loeschen
	...
	case 'addgroup':
		...
		$ad_groups_tpl = new template; // Neues Template-Objekt erstellen
		$ad_groups_tpl->load($template_path.'/'.$tpl_config['tpl_ad_groups_addgroup']); // Template laden

		if($error != '') $ad_groups_tpl->blocks['errorrow']->parse_code(); // Falls es einen Fehler gibt
		else $ad_groups_tpl->unset_block('errorrow'); // Falls kein Fehler, Block komplett loeschen
	...
	case 'editgroup':
		...
		$ad_groups_tpl = new template; // Neues Template-Objekt erstellen
		$ad_groups_tpl->load($template_path.'/'.$tpl_config['tpl_ad_groups_editgroup']); // Template laden
	...
	case 'managemembers':
		...
		$ad_groups_tpl = new template; // Neues Template-Objekt erstellen
		$ad_groups_tpl->load($template_path.'/'.$tpl_config['tpl_ad_groups_managemembers']); // Template laden
