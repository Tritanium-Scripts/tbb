...
<p class="cellButtons"><input class="formBButton" type="submit" value="{Language::getInstance()->getString('delete_selected_newsletter')}" onclick="return confirm('{Language::getInstance()->getString('really_delete_selected_newsletter')}');" />{plugin_hook hook=PlugIns::HOOK_TPL_NEWSLETTER_LETTERS_BUTTONS}</p>
</form>{/if}