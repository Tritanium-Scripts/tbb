...
{foreach $ipBlocks as $curIPBlock}
 <tr>
  ...
  <td class="cellAlt" style="text-align:center;"><span class="fontSmall"><a href="{$smarty.const.INDEXFILE}?faction=ad_ip&amp;mode=kill&amp;id={$curIPBlock[3]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/computer_delete.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('delete')}</a></span></td>
 </tr>
{foreachelse}
...
<table class="tableStd" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr><td class="cellStd"><span class="fontNorm"><a href="{$smarty.const.INDEXFILE}?faction=ad_ip&amp;mode=new{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/computer_add.png" alt="" class="imageIcon" /> {$modules.Language->getString('add_new_ip_block')}</a></span></td></tr>
</table>
