...
<p class="cellButtons"><input class="formBButton" type="submit" value="{Language::getInstance()->getString('member_search')}" />&nbsp;&nbsp;&nbsp;<input class="formButton" type="reset" /></p>
</form>
...
<table class="tableStd" cellpadding="{Config::getInstance()->getCfgVal('tpadding')}" cellspacing="{Config::getInstance()->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr><td class="cellStd"><span class="fontNorm"><a href="{$smarty.const.INDEXFILE}?faction=ad_user&amp;mode=new{$smarty.const.SID_AMPER}"><img src="{Template::getInstance()->getTplDir()}images/icons/user_add.png" alt="{Language::getInstance()->getString('add_new_member')}" class="imageIcon" /> {Language::getInstance()->getString('add_new_member')}</a></span></td></tr>
</table>
