...
{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_RANK_RANKS_TABLE_HEAD}
...
{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_RANK_RANKS_TABLE_BODY}
...
<table class="tableStd" cellpadding="{Config::getInstance()->getCfgVal('tpadding')}" cellspacing="{Config::getInstance()->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr><td class="cellStd"><span class="fontNorm"><a href="{$smarty.const.INDEXFILE}?faction=ad_rank&amp;mode=new{$smarty.const.SID_AMPER}"><img src="{Template::getInstance()->getTplDir()}images/icons/award_star_add.png" alt="{Language::getInstance()->getString('add_new_rank')}" class="imageIcon" /> {Language::getInstance()->getString('add_new_rank')}</a>{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_RANK_RANKS_OPTIONS}</span></td></tr>
</table>
