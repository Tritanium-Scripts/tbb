...
{plugin_hook hook=PlugIns::HOOK_TPL_POSTING_EDIT_POST_FORM_START}
 ...
 <tr>
  ...
  <td class="cellAlt">{if !empty($prefixes)}<select class="formSelect" name="prefixId" class="fontNorm"><option>{Language::getInstance()->getString('prefix')}</option>{foreach $prefixes as $curPrefix}<option value="{$curPrefix[0]}"{if $curPrefix[0] == $editPost.prefixId} selected="selected"{/if}{if !empty($curPrefix[2])} style="color:{$curPrefix[2]};"{/if}>{$curPrefix[1]}</option>{/foreach}</select> {/if}<input class="formText" type="text" size="65" name="title" value="{$editPost.title}" /></td>
