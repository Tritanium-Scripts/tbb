...
{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_FORUM_EDIT_FORUM_FORM_START}
 ...
 <tr>
  ...
  <td class="cellAlt"><input class="formText" type="text" size="50" name="image" value="{$editImage}" /> <span class="fontSmall">{Language::getInstance()->getString('url_or_path')}</span></td>
 </tr>
 <tr>
  <td class="cellStd"><span class="fontNorm">{Language::getInstance()->getString('moderators_colon')}</span></td>
  <td class="cellAlt"><input class="formText" type="text" size="10" name="mods" value="{$editModIDs|join:','}" /> <span class="fontSmall">{Language::getInstance()->getString('separate_mod_ids_with_comma')}</span></td>
 </tr>
 <tr>
