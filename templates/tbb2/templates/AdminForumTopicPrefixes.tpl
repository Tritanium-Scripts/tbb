...
<table class="tableStd" cellpadding="{Config::getInstance()->getCfgVal('tpadding')}" cellspacing="{Config::getInstance()->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr><td class="cellStd"><span class="fontNorm"><a href="{$smarty.const.INDEXFILE}?faction=ad_forum&amp;mode=newTopicPrefix&amp;forum_id={$forumID}{$smarty.const.SID_AMPER}">{Language::getInstance()->getString('add_new_topic_prefix')}</a>{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_FORUM_TOPIC_PREFIXES_OPTIONS}</span></td></tr>
</table>
