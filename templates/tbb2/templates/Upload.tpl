...
{plugin_hook hook=PlugIns::HOOK_TPL_UPLOAD_FORM_START}
 <tr><td class="cellStd"><span class="fontNorm">{Language::getInstance()->getString('allowed_extensions_colon')}</span></td><td class="cellAlt"><span class="fontNorm">{if $allowedExtensions == false}<span style="font-style:italic;">{Language::getInstance()->getString('no_limitation')}</span>{else}{$allowedExtensions|join:', '}{/if}</span></td></tr>
