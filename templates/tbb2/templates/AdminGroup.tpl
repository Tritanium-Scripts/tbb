...
{foreach $groups as $curGroup}
 <tr>
  ...
  <td class="cellAlt" style="text-align:center; vertical-align:top;"><span class="fontSmall"><a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=kill&amp;group_id={$curGroup[0]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/group_delete.png" alt="{$modules.Language->getString('delete')}" style="vertical-align:middle;" /> {$modules.Language->getString('delete')}</a> | <a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=edit&amp;group_id={$curGroup[0]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/group_edit.png" alt="{$modules.Language->getString('edit')}" style="vertical-align:middle;" /> {$modules.Language->getString('edit')}</a></span></td>
 </tr>
{foreachelse}
...
<table class="tableStd" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr><td class="cellStd"><span class="fontNorm"><a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=new{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/group_add.png" alt="{$modules.Language->getString('add_new_group')}" class="imageIcon" /> {$modules.Language->getString('add_new_group')}</a></span></td></tr>
</table>
