...
{foreach $groups as $curGroup}
 <tr>
  ...
  <td class="cellAlt" style="text-align:right; vertical-align:top;"><span class="fontSmall"><a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=kill&amp;group_id={$curGroup[0]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/group_delete.png" alt="{$modules.Language->getString('delete')}" /></a> <a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=edit&amp;group_id={$curGroup[0]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/group_edit.png" alt="{$modules.Language->getString('edit')}" /></a></span></td>
 </tr>
{foreachelse}
