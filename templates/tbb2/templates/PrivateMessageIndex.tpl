...
<table class="tableStd" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:100%;">
 ...
 <tr>
  ...
  <td class="cellAlt" style="text-align:center;"><span class="fontSmall"><a href="{$smarty.const.INDEXFILE}?faction=pm&amp;mode=kill&amp;pm_id={$curPM[0]}&amp;pmbox_id={$pmBoxID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/pm_delete.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('delete')}</a> <a href="{$smarty.const.INDEXFILE}?faction=pm&amp;pmbox_id={$pmBoxID}&amp;mode=reply&amp;pm_id={$curPM[0]}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/pm_reply.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('reply')}</a></span></td>
 </tr>
{foreachelse}
