...
{foreach $topics as $curTopic}
 <tr onmouseover="setRowCellsClass(this, 'cellHighlight');" onmouseout="restoreRowCellsClass(this);">
  ...
  <td class="cellStd"><span class="topicLink">{if $curTopic.isMoved}<span style="font-weight:bold;">{Language::getInstance()->getString('moved_colon')}</span> {/if}{if $curTopic.isSticky}<b>{Language::getInstance()->getString('sticker_colon')|upper}</b> {/if}{if $curTopic.isPoll}{Language::getInstance()->getString('poll_colon')} {/if}{if !empty($curTopic.topicPrefix)}<span style="font-weight:bold;{if !empty($curTopic.topicPrefix.color)} color:{$curTopic.topicPrefix.color};{/if}">{$curTopic.topicPrefix.prefix}</span> {/if}<a class="topicLink" href="{$smarty.const.INDEXFILE}?mode=viewthread&amp;forum_id={if $curTopic.isMoved}{$curTopic.movedForumID}{else}{$forumID}{/if}&amp;thread={if $curTopic.isMoved}{$curTopic.movedTopicID}{else}{$curTopic.topicID}{/if}{$smarty.const.SID_AMPER}">{$curTopic.topicTitle}</a></span> <span class="fontSmall">{$curTopic.topicPageBar}</span></td>
