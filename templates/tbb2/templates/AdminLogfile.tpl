...
{foreach $logfiles as $curLogfile}
 <tr>
  ...
  <td class="cellStd" style="text-align:center;"><span class="fontSmall"><a href="{$smarty.const.INDEXFILE}?faction=adminLogfile&amp;mode=view&amp;log={$curLogfile.name}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/report.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('view')}</a> | {if $curLogfile.isDeletable}<a href="{$smarty.const.INDEXFILE}?faction=adminLogfile&amp;mode=delete&amp;log={$curLogfile.name}{$smarty.const.SID_AMPER}" onclick="return confirm('{$modules.Language->getString('really_delete_this_logfile')}');"><img src="{$modules.Template->getTplDir()}images/icons/report_delete.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('delete')}</a> | {/if}<a href="{$smarty.const.INDEXFILE}?faction=adminLogfile&amp;mode=download&amp;log={$curLogfile.name}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/icons/report_disk.png" alt="" style="vertical-align:middle;" /> {$modules.Language->getString('download')}</a></span></td>
 </tr>
{foreachelse}
