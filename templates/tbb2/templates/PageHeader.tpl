    ...
    <td class="cellNavBarBig">
     <table border="0" cellpadding="0" cellspacing="0" style="width:100%;">
      <tr>
       ...
       <td style="text-align:right;"><span class="fontNavBar">{if $smarty.config.debug}&nbsp;{$action} &ndash; {$subAction}{/if}{if ($action == 'Forum' || $action == 'PostNew' || $action == 'Posting') && $subAction != 'ForumIndex' && $subAction != 'ViewTodaysPosts' && $subAction != 'Message'}<a href="{$smarty.const.INDEXFILE}?faction=newtopic&amp;forum_id={$forumID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/new_topic.png" alt="" class="imageButton" /></a>&nbsp;<a href="{$smarty.const.INDEXFILE}?faction=newpoll&amp;forum_id={$forumID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/new_poll.png" alt="" class="imageButton" /></a>{if $subAction == 'ViewTopic'}&nbsp;<a href="{$smarty.const.INDEXFILE}?faction=reply&amp;forum_id={$forumID}&amp;thread_id={$topicID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/new_reply.png" alt="" class="imageButton" /></a>{/if}{elseif $action == 'PrivateMessage' && $subAction != 'Message'}<a href="{$smarty.const.INDEXFILE}?faction=pm&amp;pmbox_id={$pmBoxID}&amp;mode=send{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/new_pm.png" alt="" class="imageButton" /></a>{if $subAction == 'PrivateMessageViewPM' || $subAction == 'PrivateMessageConfirmDelete'}&nbsp;<a href="{$smarty.const.INDEXFILE}?faction=pm&amp;pmbox_id={$pmBoxID}&amp;mode=reply&amp;pm_id={$pmID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/pm_new_reply.png" alt="" class="imageButton" /></a>&nbsp;<a href="{$smarty.const.INDEXFILE}?faction=pm&amp;pmbox_id={$pmBoxID}&amp;mode=kill&amp;pm_id={$pmID}{$smarty.const.SID_AMPER}"><img src="{$modules.Template->getTplDir()}images/buttons/pm_delete.png" alt="" class="imageButton" /></a>{/if}{else}&nbsp;{/if}</span></td>
      </tr>
     </table>
    </td>
   </tr>
