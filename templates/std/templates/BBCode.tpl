...
{/foreach}
 ...
 <div style="padding:3px;"><b>{$modules.Language->getString('hidden_text', 'BBCode')}:</b> <input class="forumcode" type="button" value="{$modules.Language->getString('uncover', 'BBCode')}" onclick="(s = this.parentNode.parentNode.getElementsByTagName('div')[1].style).display = s.display == 'none' ? '' : 'none'; (s = this.parentNode.style).backgroundColor = s.backgroundColor == '' ? '#666699' : ''; s.color = s.color == '' ? '#FFFFFF' : ''; this.value = s.color == '' ? '{$modules.Language->getString('uncover', 'BBCode')}' : '{$modules.Language->getString('hide', 'BBCode')}';" /></div>
 ...
 <div style="background-color:#666699; color:#FFFFFF; font-weight:bold; padding:3px;">{$modules.Language->getString('locked_text', 'BBCode')}:</div>
 <div style="border:1px #000000 solid; padding:10px; background-color:#{if $lockText != ''}F0FFF0;">{$lockText}{else}FFD1D1;"><span style="color:red; font-weight:bold;">{$modules.Language->getString('locked_text_hint', 'BBCode')}</span>{/if}</div>
