...
</div>{elseif $type == $smarty.const.BBCODE_CENTER}<p style="text-align:center;">{$centerText}</p>{elseif $type == $smarty.const.BBCODE_EMAIL}{mailto address=$eMailAddress text=$eMailText encode="javascript"}{elseif $type == $smarty.const.BBCODE_IMAGE}<img src="{$imageAddress}" alt="{$imageText}" title="{$imageText}" />{elseif $type == $smarty.const.BBCODE_LINK}<a href="{$linkAddress}" target="_blank">{$linkText}</a>{elseif $type == $smarty.const.BBCODE_COLOR}<span style="color:{$colorCode};">{$colorText}</span>{elseif $type == $smarty.const.BBCODE_IFRAME}<iframe src="{$iFrameLink}" width="{$iFrameWidth}" height="{$iFrameHeight}" frameborder="0"><a href="{$iFrameLink}" target="_blank">{Language::getInstance()->getString('no_iframe_supported', 'BBCode')}</a></iframe>{elseif $type == $smarty.const.BBCODE_SIZE}<span style="font-size:{$sizeFont};">{$sizeText}</span>{elseif $type == $smarty.const.BBCODE_GLOW}<div style="width:100%; filter:Glow(color={$glowColor}, strength=4);">{$glowText}</div>{* <span> doesn't work?! *}{elseif $type == $smarty.const.BBCODE_SHADOW}<div style="width:100%; filter:DropShadow(color={$shadowColor}, offx=2, offy=2);">{$shadowText}</div>{* <span> doesn't work?! *}{elseif $type == $smarty.const.BBCODE_FLASH}<object data="{$flashLink}" type="application/x-shockwave-flash" width="{$flashWidth}" height="{$flashHeight}">
