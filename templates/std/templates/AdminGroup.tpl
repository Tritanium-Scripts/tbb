...
{foreach $groups as $curGroup}
 <tr>
  ...
  <td class="td2" style="vertical-align:top;"><span class="small">{if !empty($curGroup[3])}{$curGroup[3]|join:', '}{else}{Language::getInstance()->getString('no_members')}{/if}</span></td>
...
<p class="norm"><a href="{$smarty.const.INDEXFILE}?faction=ad_groups&amp;mode=new{$smarty.const.SID_AMPER}">{Language::getInstance()->getString('add_new_group')}</a>{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_GROUP_GROUPS_OPTIONS}</p>