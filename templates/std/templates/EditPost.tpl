...
<table class="tbl" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:{$modules.Config->getCfgVal('twidth')}; margin:auto;">
 ...
 <tr>
  ...
  <td class="td1" style="width:80%;">
   ...
   <input type="checkbox" id="isAddURLs" name="isAddURLs" value="true" style="vertical-align:middle;"{if $editPost.isAddURLs} checked="checked"{/if} /> <label for="isAddURLs" class="norm">{$modules.Language->getString('auto_transform_links')}</label>{/if}<br />
   <input type="checkbox" id="isLastEditBy" name="isLastEditBy" value="true" style="vertical-align:middle;"{if $editPost.isLastEditBy} checked="checked"{/if} /> <label for="isLastEditBy" class="norm">{$modules.Language->getString('show_as_last_editor')}</label>
  </td>
 </tr>
</table>
