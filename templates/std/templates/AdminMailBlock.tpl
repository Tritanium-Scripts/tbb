...
<p class="norm"><a class="norm" href="{$smarty.const.INDEXFILE}?faction=adminMailBlock&amp;mode=new{$smarty.const.SID_AMPER}">{Language::getInstance()->getString('add_new_mail_block')}</a>{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_MAIL_BLOCK_BLOCKS_OPTIONS}</p>