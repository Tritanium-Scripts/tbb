...
{else}
    ...
    <li style="list-style-image:url('{$curEvent.icon}'); list-style-position:inside;" class="norm" title="{$curEvent.name}"><a style="cursor:help;" onclick="document.getElementById('event{$curDay}{$curEvent@iteration}').style.display='';">{$curEvent.name}</a></li>
{/if}
{/foreach}
   </ul>
{/if}
