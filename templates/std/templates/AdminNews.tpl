...
<table class="tbl" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:{$modules.Config->getCfgVal('twidth')}; margin:auto;">
 ...
 <tr>
  ...
  <td class="td1"><select name="expiredate"><option value="-1"{if $newsDuration == -1} selected="selected"{/if}>{$modules.Language->getString('forever')}</option><option value="60"{if $newsDuration == 60} selected="selected"{/if}>{$modules.Language->getString('one_hour')}</option><option value="120"{if $newsDuration == 120} selected="selected"{/if}>{2|string_format:$modules.Language->getString('x_hours')}</option><option value="300"{if $newsDuration == 300} selected="selected"{/if}>{5|string_format:$modules.Language->getString('x_hours')}</option><option value="1440"{if $newsDuration == 1440} selected="selected"{/if}>{$modules.Language->getString('one_day')}</option><option value="2880"{if $newsDuration == 2880} selected="selected"{/if}>{2|string_format:$modules.Language->getString('x_days')}</option><option value="7200"{if $newsDuration == 7200} selected="selected"{/if}>{5|string_format:$modules.Language->getString('x_days')}</option><option value="14400"{if $newsDuration == 14400} selected="selected"{/if}>{10|string_format:$modules.Language->getString('x_days')}</option><option value="43200"{if $newsDuration == 43200} selected="selected"{/if}>{30|string_format:$modules.Language->getString('x_days')}</option></td>
 </tr>
