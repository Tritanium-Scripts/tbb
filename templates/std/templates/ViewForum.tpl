...
{foreach $topics as $curTopic}
 <tr>
  ...
  <td class="td1"><span class="topiclink">{if $curTopic.isMoved}<span style="font-weight:bold;">{Language::getInstance()->getString('moved_colon')}</span> {/if}{if $curTopic.isSticky}<b>{Language::getInstance()->getString('sticker_colon')|upper}</b> {/if}{if $curTopic.isPoll}{Language::getInstance()->getString('poll_colon')} {/if}<a class="topiclink" href="{$smarty.const.INDEXFILE}?mode=viewthread&amp;forum_id={if $curTopic.isMoved}{$curTopic.movedForumID}{else}{$forumID}{/if}&amp;thread={if $curTopic.isMoved}{$curTopic.movedTopicID}{else}{$curTopic.topicID}{/if}{$smarty.const.SID_AMPER}">{$curTopic.topicTitle}</a></span> <span class="small">{$curTopic.topicPageBar}</span></td>
