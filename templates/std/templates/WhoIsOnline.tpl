...
{* 0:user - 1:location - 2:isGhost - 3:xMinAgo *}
 <tr><td class="td1"><span class="norm">{if $curLocation[2]}<img src="{$modules.Template->getTplDir()}images/ghost.png" alt="{$modules.Language->getString('browses_as_ghost')}" title="{$modules.Language->getString('browses_as_ghost')}" style="vertical-align:top;" /> {/if}{$curLocation[0]}</span></td><td class="td2"><span class="norm">{$curLocation[1]}</span></td><td class="td2" style="text-align:right;"><span class="norm">{$curLocation[3]}</span></td></tr>
{foreachelse}
