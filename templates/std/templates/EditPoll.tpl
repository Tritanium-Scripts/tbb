...
<p style="text-align:center;"><input type="submit" name="update" value="{Language::getInstance()->getString('edit_poll')}" />&nbsp;&nbsp;&nbsp;<input type="submit" name="{if $isClosed}open" value="{Language::getInstance()->getString('open_poll')}{else}close" value="{Language::getInstance()->getString('close_poll')}{/if}" />{plugin_hook hook=PlugIns::HOOK_TPL_POSTING_EDIT_POLL_BUTTONS}</p>
</form>