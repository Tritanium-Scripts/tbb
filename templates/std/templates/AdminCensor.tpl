...
<p class="norm"><a class="norm" href="{$smarty.const.INDEXFILE}?faction=ad_censor&amp;mode=new{$smarty.const.SID_AMPER}">{Language::getInstance()->getString('add_new_censorship')}</a>{plugin_hook hook=PlugIns::HOOK_TPL_ADMIN_CENSOR_CENSORSHIPS_OPTIONS}</p>