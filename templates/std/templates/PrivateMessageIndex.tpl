...
<table class="tbl" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:{$modules.Config->getCfgVal('twidth')}; margin:auto;">
 <tr>
  ...
  <th class="thsmall"><span class="thsmall">{if $isOutbox}{$modules.Language->getString('to')}{else}{$modules.Language->getString('from')}{/if}</span></th>
 ...
 <tr>
  ...
  <td class="td1" style="text-align:center;"><span class="small"><a href="{$smarty.const.INDEXFILE}?faction=pm&amp;mode=kill&amp;pm_id={$curPM[0]}&amp;pmbox_id={$pmBoxID}{$urlSuffix}{$smarty.const.SID_AMPER}">{$modules.Language->getString('delete')}</a>{if !$isOutbox} | <a href="{$smarty.const.INDEXFILE}?faction=pm&amp;pmbox_id={$pmBoxID}&amp;mode=reply&amp;pm_id={$curPM[0]}{$smarty.const.SID_AMPER}">{$modules.Language->getString('reply')}</a>{/if}</span></td>
 </tr>
{foreachelse}
...
{if count($pms) > 0}<p style="text-align:center;"><input type="submit" value="{$modules.Language->getString('delete_selected_pms')}" /></p>{/if}
</form>