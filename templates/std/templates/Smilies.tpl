...
{if $modules.Auth->isAdmin() || $isMod}{foreach Main::getModule('BBCode')->getAdminSmilies() as $curSynonym => $curSmiley} <a href="javascript:setTag(' {$curSynonym}', '');">{$curSmiley}</a>{/foreach}<hr />{/if}
