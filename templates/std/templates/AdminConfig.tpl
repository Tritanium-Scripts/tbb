...
<table class="tbl" cellpadding="{$modules.Config->getCfgVal('tpadding')}" cellspacing="{$modules.Config->getCfgVal('tspacing')}" style="width:{$modules.Config->getCfgVal('twidth')}; margin:auto;">
 ...
 <tr>
  ...
  <td class="td1" style="vertical-align:top; width:65%;"><select name="settings[50]">{foreach $modules.Language->getAvailLangs() as $curLangCode}<option value="{$curLangCode}"{if $curLangCode == $configValues['lng_folder']} selected="selected"{/if}>{$curLangCode}</option>{/foreach}</select></td>
 </tr>
 <tr>
