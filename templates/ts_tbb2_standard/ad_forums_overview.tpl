...
<tr><td class="celltitle" colspan="2"><span class="fonttitle">{$LNG['Forums_without_category']}</span></td></tr>
 <template:forumrow>
 <tr>
  <td class="{$akt_cell_class}"><span class="fontnorm">{$cur_forum['forum_name']}</span></td>
  <td class="{$akt_cell_class}" align="right"><span class="fontsmall"><a href="administration.php?faction=ad_forums&amp;mode=editsrights&amp;forum_id={$cur_forum['forum_id']}&amp;{$MYSID}">{$LNG['Edit_special_rights']}</a> | <span class="fontsmall"><a href="administration.php?faction=ad_forums&amp;mode=editforum&amp;forum_id={$cur_forum['forum_id']}&amp;{$MYSID}">{$LNG['Edit']}</a></span></td>
 </tr>
 </template>
</table>
<br />
