...
<tr><th class="thnorm" colspan="2"><span class="thnorm">{$lng["Edit_forum"]}</span></th></tr>
<!-- TPLBLOCK errorrow -->
...
<tr>
 ...
 <td class="td1" width="85%"><select class="form_select" name="p_cat_id">
  <option value="0"{$no_cat_selected}>{$lng["No_category"]}</option>
 <!-- TPLBLOCK optionrow -->
  <option value="{optionrow.$akt_cat["cat_id"]}"{optionrow.$selected}>{optionrow.$akt_cat["cat_name"]}</option>
 <!-- /TPLBLOCK optionrow -->
 </select></td>
</tr>
...
<tr><td colspan="2" class="buttonrow" align="center"><input type="submit" class="form_bbutton" value="{$lng["Edit_forum"]}" />&nbsp;&nbsp;&nbsp;<input class="form_button" type="reset" value="{$lng["Reset"]}" /></td></tr>
</table>
</form>