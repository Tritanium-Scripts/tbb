  ...
  <td class="cellalt" width="85%" valign="middle">
   ...
   <tr>
    ...
    <td align="right">
     {$delete_button}
     {$edit_button}
     <if:"{$akt_post['poster_id']} != 0"><if:"{$akt_post['poster_hide_email']} != 1"><a href="mailto:{$akt_post['poster_email']}"><img src="{$TEMPLATE_PATH}/{$TCONFIG['images']['user_email']}" alt="{$akt_post['poster_email']}" border="0" /></a><else /><if:"{$akt_post['poster_receive_emails']} == 1"><a href="http://helios/tbb2/index.php?faction=viewprofile&profile_id={$akt_post['poster_id']}&mode=sendmail&amp;{$MYSID}"><img src="{$TEMPLATE_PATH}/{$TCONFIG['images']['user_email']}" alt="{$LNG['Send_email']}" border="0" /></a></if></if></if>
     {$user_hp_button}
     {$quote_button}
    </td>
   </tr>
